...
router.post('/password', checkAuth, function (req, res, next) {
...
router.post('/information', checkAuth, function (req, res, next) {
...
/*c创建团队*/
router.post('/create_team', checkAuth, function (req, res, next) {
  //创建团队-》在学生团队表插入信息
  let stuId = req.session.stuId
  if (!stuId && stuId != 0) {
    res.status(500).send()
...
/*申请加入团队*/
router.post('/applyToteam', checkAuth, function (req, res, next) {
...
/*管理团队申报！！！*/
router.post('/manageApply', checkAuth, function (req, res) {
...
router.get('/myteams', checkAuth, function (req, res, next) {
  let stuId = req.session.stuId
  let teams = null
  if (!stuId && stuId != 0) {
    res.status(500).send({
...
* 就插入一条新数据
* */
router.post('/PapplyToComp', checkAuth, function (req, res) {
  let stuId = req.session.stuId
...
已结束：3
* */
router.get('/myComp', checkAuth, function (req, res, next) {
  //逻辑：我参与的竞赛团队对应的竞赛
...
/*退出团队*/
router.post('/quit_team', checkAuth, function (req, res) {
  let stuId = req.session.stuId
  let teamId = req.body.teamId
  if (!stuId || !teamId) {
    res.status(500).send()
...
router.post("/uploadWorks", checkAuth, (req, res) => {
